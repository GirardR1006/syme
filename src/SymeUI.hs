...
-- UI constructor
    builder <- builderNew
    builderAddFromFile builder "syme-work/SymeUI.glade"
--Windows and dialog definition
    ...
    dialogcsv <- builderGetObject builder castToDialog "csvDialog" 
    ...
    dialogcsv `on` deleteEvent $ liftIO (widgetHide dialogcsv) >> return False
    labeldialogcsv <- builderGetObject builder castToLabel "csvDialogLabel" 
--Button definition
    ...
    qbttncsv <- builderGetObject builder castToButton "csvDialogOkBttn"
...
--runtime (on a single-thread basis), IORef monad is used.
    nm <- getMapMeth "syme-work/onche.cod"
    si <- sourcesFromFile "syme-work/sources"
    methmap<- newIORef nm
...
----Buttons
    ...
    csvbttn `on` buttonActivated $ do rf <- readIORef methmap
                                      today <- curDate
                                      let path = "export-excel/"++show(today)++".csv"
                                      saveMapInFile rf path
                                      labelSetText labeldialogcsv ("Données sauvegardées dans " ++ path)
                                      handleCsv <- dialogRun dialogcsv
                                      if handleCsv == ResponseOk
                                          then putStrLn "Ok signal received, hiding window"
                                          >> widgetHide dialogcsv
                                      else 
                                          putStrLn "Cancel signal received, hiding window."
                                          >> widgetHide dialogcsv
    ...
    qbttn `on` buttonActivated $ do putStrLn "Saving data in storage..."
                                    ...
                                    saveMapInFile rf "syme-work/onche.cod"
...
--CSV window signal
----Button
    qbttncsv `on` buttonActivated $ dialogResponse dialogcsv ResponseOk
----TreeView
